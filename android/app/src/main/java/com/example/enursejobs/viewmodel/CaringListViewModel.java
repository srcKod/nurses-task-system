...
public class CaringListViewModel extends AndroidViewModel {
    private static final String QUERY_KEY = "QUERY";
    final DataRepository mRepository;
    private final SavedStateHandle savedStateHandle;
    private final LiveData<List<CaringEntity>> mCaringEntity;
    private final MutableLiveData<Integer> mIndex = new MutableLiveData<>();
    public CaringListViewModel(@NonNull Application application,  @NonNull SavedStateHandle savedStateHandle) {
        super(application);
        ...
        mCaringEntity = Transformations.switchMap(
                savedStateHandle.getLiveData(QUERY_KEY, null),
                query -> {
                    String queryString = (String) query; // cast the query to String type
                    if (TextUtils.isEmpty(queryString)) {
                        return mRepository.getCarings();
                    }
                    return mRepository.findCarings("*" + queryString + "*");
                });
    }
    ...
    /**
     * Expose the LiveData Carings query so the UI can observe it.
     */
    public LiveData<List<CaringEntity>> getCarings() {
        return mCaringEntity;
    }
}
