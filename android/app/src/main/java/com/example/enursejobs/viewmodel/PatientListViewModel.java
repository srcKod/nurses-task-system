...
public class PatientListViewModel extends AndroidViewModel {
    ...
    private final LiveData<List<PatientEntity>> mPatientEntity;
    private final MutableLiveData<Integer> mIndex = new MutableLiveData<>();

    public PatientListViewModel(@NonNull Application application,  @NonNull SavedStateHandle savedStateHandle) {
        super(application);
        ...
        mPatientEntity = Transformations.switchMap(
                savedStateHandle.getLiveData(QUERY_KEY, null),
                query -> {
                    String queryString = (String) query; // cast the query to String type
                    if (TextUtils.isEmpty(queryString)) {
                        return mRepository.getPatients();
                    }
                    return mRepository.findPatients("*" + queryString + "*");
                });
    }
    ...
    /**
     * Expose the LiveData Patients query so the UI can observe it.
     */
